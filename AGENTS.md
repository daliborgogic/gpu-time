...
- The current `serbian-v1` export records **808/1000 exact token-label and boundary matches** on reserved Serbian carriers (a cold start; there is no prior Serbian checkpoint to continue from). Held-in generated scores share rendering families with training and must not be quoted as language accuracy. See `MODEL_CARD.md`.
- When continuing `serbian-v1` (not starting a new language), train with `--storage f32 --feature-rows 324 --init runs/serbian-v1/best.pt` to avoid quantization drift. The defaults are f16, 580 feature rows, and a cold start. A cold start can lose rare terse forms.
...
The library was ported from English to Serbian (Latin script) in commit `30198e7` and its Phase 2 follow-up. The English `spoken2` lineage (22 checkpoints) was retired rather than continued; `serbian-v1` is a fresh cold start. English-language commits before the port remain in Git history but describe a different parser.
